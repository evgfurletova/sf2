...
void CalcBnp(int n){
	...
	for(j = 0; j < MainData::NOccur; j++){ 
		...
		for(i = 0; i < H_M_Node::NumAllStates; i++){ //for all q in Q
			if(MainData::order < 0){
				s = H_M_Node::ConsistStNums[i];		//for all q' s.t. п(q',q)>0
			}
			
			sum = 0; //variable to compute Sum_{q'}Prob(B(n-m-1,j),q')*п(q',q)
	        for(k = 0; k < s; k++){
				int state = H_M_Node::ConsistStMatrix[i][k];
				sum += PrevBnpProbs[state* MainData::NOccur + j]*H_M_Node::TransProbMatrix[i][k];  //Sum += Prob(B(n-m-1,j),q')*п(q',q)
			}

...
void FNonEmptyLine(ifstream *ff,char* &line){
    ...
    size_t lastChar = strlen(line);
    if (lastChar>0 && (line[lastChar-1]=='\n' || line[lastChar-1]=='\r')) {
        line[lastChar-1] = '\0';
    }
	return;
}
...
int Get_ND_HHM(ifstream *ff){
	...
	MainData::ND_HHMProbs = new double**[H_M_Node::NumAllStates];
	MainData::ND_HHMTrans = new vector<int>*[H_M_Node::NumAllStates];
	
	for(i = 0; i < H_M_Node::NumAllStates; i++){
		MainData::ND_HHMProbs[i] = new double*[H_M_Node::NumAllStates];
...
int Get_D_HHM(ifstream *ff){
	...
	FNonEmptyLine(ff,line);
	i = 0;
	while((line[0] != '-')&&(i < H_M_Node::NumAllStates)){
		j = 0;
		pch = strtok(line,delim);
...
int MainData::AToi(const char Let){
...
double CompWordProb(string &word){
	double prob = 1;
	int i;
	if(MainData::order == 0){
		for(i = 0; i < MainData::WordLen; i++)
			prob = prob*MainData::BernProb[MainData::AToi(word[i])]; 
	}
	if(MainData::order < 0){
		for(i = 0; i < H_M_Node::NumAllStates; i++)
			prob += TermProb(word, MainData::WordLen, i);
	}
	if(MainData::order > 0){
		int sufcd = MModel_Prob::SuffixN(MainData::order,word,MainData::WordLen);
		prob = TermProb(word, MainData::WordLen, sufcd);
	}
	return prob;
}
...
ifstream ff(MainData::PatternFileName.c_str());
	...
	while (strlen(line) != 0){
		pch = strtok(line,delim);
        if(strlen(pch) == size_t(MainData::WordLen)){
			for(j = 0; j< MainData::WordLen; j++){
				k = MainData::AToi(pch[j]);
				if(k != -1){
					DigitLine[j] = k;
...
//It gives 1 if the word is in the trie else -0
 int In_Trie(int *word){
	 ...
	 int k;
	 int i;
	 NodeAC *T = NodeAC::ACRoot;

	 for(i = 0; i < MainData::WordLen; i++){
		 if(T->leaf == 0){
			InternAC* T1 = static_cast<InternAC*>(T);
			k = word[i]; 
			if(T1->LChilds[k] == NULL) return 0;
	...
	for(j = 0; j < MainData::AlpSize; j++){
		...
		if(i == (MainData::WordLen - 1)){
			...
			if((score > MainData::Thr)||(score == MainData::Thr)){
				MainData::NWords++;
				NodeAC::ACRoot->InsertWord(word, MainData::NWords);
			}
		} 
		else{
...
int MainData::GenPssmWords1(NodeAC* node, int *word, string& stword, int i, double score, double* SMas){
	...
	for(j = 0; j < MainData::AlpSize; j++){
		...
		if(i == (MainData::WordLen - 1)){ //if new node is leaf (a word from motif)
			...
			if((ms > MainData::Thr)||(ms == MainData::Thr)){
				...
				MainData::GenPssmWords1(node, word, stword,i+1, score, SMas);
			}
		}
	}
	return 0;
...
//It adds new node to trie
void AddNodeMV(NodeAC* &node, int pos, int* word){
	if(pos < MainData::WordLen -1){
			NodeAC::InsertNode(node,word[pos],0);
			
	}
	if(pos == MainData::WordLen -1){
			MainData::NWords++;
			NodeAC::InsertNode(node,word[pos],MainData::NWords);
	}
}


//Generates all words that are defferent from a geven word H in geven number of positions
void MainData::MotifVariations1(NodeAC* node, int num, int pos, int* word){
	//Here: 1) "num" is number of mismatches; 2) "pos" number of position in word; "word" is word corresponding to the current node
	//The procedure generates new node of length pos
	int i;	
	NodeAC* node1 = node;
	if((num > 0)&&(pos < MainData::WordLen)){    
		if((MainData::ConstPositions == NULL)||(MainData::ConstPositions[pos] == 0)){ //the position 
			int Let = word[pos];
		//generates new node of length pos
			for( i = 0; i < MainData::AlpSize; i++){
				node = node1;
				if(Let != i){
					word[pos] = i;
					node = node1;
					AddNodeMV(node,pos,word);
					if(pos < MainData::WordLen -1) MainData::MotifVariations1(node, num -1, pos + 1, word);
					word[pos] = Let;
					node = node1;
				}else{
					AddNodeMV(node,pos,word);
					if(pos < MainData::WordLen -1) MainData::MotifVariations1(node, num, pos + 1, word);
					node = node1;
				}
			}
		}/*if((MainData::ConstPositions == NULL)||(MainData::ConstPositions[pos] == 0))*/
		else{
			AddNodeMV(node,pos,word);
			MainData::MotifVariations1(node, num, pos + 1, word);
			node = node1;
		}
	} /*if((num > 0)&&(pos < MainData::WordLen))*/
	else{
...
//gives all words satisfying to the consensus
int MainData::ConsVariations1(NodeAC* node, int pos, int* word){
	
	NodeAC* node1 = node;
	int pos1 = MainData::consensus[pos];
	int s = (int)MainData::ConsAlp[pos1].size();
	int i;
	for(i = 1; i < s; i++){
		node = node1;
		...
		if(pos < MainData::WordLen - 1){
			NodeAC::InsertNode(node, word[pos], 0);
			MainData::ConsVariations1(node, pos + 1,word);
		}else{
			MainData::NWords++;
			NodeAC::InsertNode(node, word[pos], MainData::NWords);
		}
...
//		NodeAC::ACRoot->InsertWord(motif, NWords);
	...
	if(MainData::mode == 5){
		...
		int* word = new int[WordLen + 1];
		word[WordLen] = -1;
		i = ConsVariations1(NodeAC::ACRoot, 0, word);
		if(i > 0){
			return i;
		}
		delete[] word;
...
void MainData::PrintMain(ostream *ff,int exitflag){
	...
	*(ff)<<"\tProgram SufPref, version 2.1\n";
	...
	*(ff)<<"\n 2. Results \n";
	if(exitflag == 0){
		*(ff)<<"Number of overlap classes: "<<NodeOv::NClasses<<"\n";
		*(ff)<<"Number of overlaps: "<<NodeOv::NumOVNodes<<'\n';
		*(ff)<<"Number of  nodes in AC trie: "<<NodeAC::NumACNodes<<"\n";
	}
	*(ff)<<"p-Value: ";
	*(ff)<<MainData::Pvalue;
	return;

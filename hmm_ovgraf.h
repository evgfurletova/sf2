...
public:
	...
	static void States_and_BackProbs(H_M_Node* node, H_M_Node* LPnode, string &back, double* PredProbs, int* PredStates, double* NewProbs, int* NewStates, double** BackP);	//for all w in OV(H) create descriptors of states form AllStates(w) and for all q' in AllStates(w) and q in AllStates(w) compute Prob(q',back,q) 
